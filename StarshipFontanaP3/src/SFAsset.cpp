...
SFAsset::SFAsset(SFASSETTYPE type, std::shared_ptr<SFWindow> window): type(type), sf_window(window) {
  ...
  case SFASSET_ALIEN:
    ...
    break;
case SFASSET_WALL:
    sprite = IMG_LoadTexture(sf_window->getRenderer(), "assets/wall.png");//zhe edited here
    break;
  case SFASSET_COIN:
...
void SFAsset::GoNorth() {
  Vector2 c = *(bbox->centre) + Vector2(0.0f, 5.0f);// up speed
  bbox->centre.reset();
  bbox->centre = make_shared<Vector2>(c);
}

void SFAsset::GoSouth() {
  Vector2 c = *(bbox->centre) + Vector2(0.0f, -5.0f);// down speed
  bbox->centre.reset();
  bbox->centre = make_shared<Vector2>(c);
}

void SFAsset::Stop() {
  Vector2 c = *(bbox->centre) + Vector2(0.0f, 0.0f);// stop
  bbox->centre.reset();
  bbox->centre = make_shared<Vector2>(c);
}
