...
SFApp::SFApp(std::shared_ptr<SFWindow> window) : fire(0),score(0), is_running(true), sf_window(window) {
  ...
  for(int i=0; i<number_of_walls; i++) {
    ...
    auto pos   = Point2(((canvas_w/number_of_walls) * i)+ 30, 100.0f);
    wall->SetPosition(pos);
    walls.push_back(wall);
  }

  auto coin = make_shared<SFAsset>(SFASSET_COIN, sf_window);
  auto pos  = Point2((canvas_w/4)+30, 300);
...
void SFApp::OnEvent(SFEvent& event) {
  ...
  case SFEVENT_UPDATE:
    ...
    OnRender();
    break;
  case SFEVENT_PLAYER_UP://Zhe edited here
    player->GoNorth();
    break;
  case SFEVENT_PLAYER_DOWN://Zhe edited here
    player->GoSouth();
    break;
...
void SFApp::OnRender() {
  ...
  for(auto w: walls) {//zhe edited here
    if(w->IsAlive()) {w->OnRender();}//render wall
  }
